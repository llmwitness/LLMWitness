...
from utils import generate_uuidv7, redact_pii
...
class AgentTraceTracker:
    ...
    def wrap_openai_client(self, client: Any) -> Any:
        ...
        def intercepted_create(*args, **kwargs):
            ...
            # Attach header if headers parameter supported
            ...
            try:
                ...
                print(f"[AgentTrace SDK Warning] OpenAI wrapper parsing error: {err}")
    ...
    def flush(self):
        """Blocks until all queued telemetry items are processed."""
        self.queue.join()

    def shutdown(self):
        """Flushes queue and safely stops background thread worker."""
        self.flush()
        self.running = False
...
# Global SDK instance
default_tracker = AgentTraceTracker()


def trace_session(task_name: str, correlation_id: Optional[str] = None):
    """Convenience global context manager."""
    return default_tracker.trace_session(task_name, correlation_id)

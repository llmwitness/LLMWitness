...
The wheel contains the `llmwitness` Python package, license files, and metadata. The source distribution additionally contains `llmwitness.js`, extension assets, and project documentation. Tests, benchmark scripts, local receipts, caches, databases, logs, secrets, and generated reports must not be included in either archive.

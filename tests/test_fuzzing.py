"""
LLMWitness Community - Fuzz Testing Suite
...
def generate_fuzz_payload(depth: int = 0) -> any:
    if depth > 4 or random.random() < 0.3:
        ...
        if choice == "ssn":
            ...
            return (
                f"CC: 4111-{random.randint(1000,9999)}-{random.randint(1000,9999)}-4444"
            )
        elif choice == "token":
            return f"Bearer sk-{random_string(30)}"
        elif choice == "b64":
            return "data:image/png;base64," + base64.b64encode(
                os.urandom(random.randint(10, 2000))
            ).decode("utf-8")
        elif choice == "text":
        ...
        return {
            f"key_{random_string(5)}": generate_fuzz_payload(depth + 1)
...
def test_pii_engine_fuzzing():
    ...
    for _ in range(100):
        ...
        try:
            ...
            pytest.fail(
                f"PII Redaction crashed during fuzzing on payload: {payload}. Error: {e}"
            )

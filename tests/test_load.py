"""
AgentTrace Production Testing - Load Testing Suite
Executes high-concurrency multi-threaded load tests against Gateway and Ingestion servers.
"""
...
def gateway_server():
    ...
    p1 = subprocess.Popen([sys.executable, "-m", "uvicorn", "ingest:app", "--host", "127.0.0.1", "--port", str(ingest_port)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=env)
    p2 = subprocess.Popen([sys.executable, "-m", "uvicorn", "gateway:app", "--host", "127.0.0.1", "--port", str(gateway_port)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=env)

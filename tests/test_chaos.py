"""
AgentTrace Production Testing - Chaos Testing Suite
Simulates ingestion service failure, network timeout, corrupted packet payloads, and WORM vault tampering.
"""
...
def test_gateway_fail_open_on_ingestion_crash():
    ...
    cmd = [sys.executable, "-m", "uvicorn", "gateway:app", "--host", "127.0.0.1", "--port", str(gateway_port)]
    ...
    try:
        ...
        res = client.post(
            ...
            json={"model": "gpt-4o", "messages": [{"role": "user", "content": "Chaos test request"}]}
        )
        assert res.status_code == 200, f"Gateway failed open requirement: status {res.status_code}"
    finally:
        client.close()
        proc.terminate()
        try:
...
def test_worm_vault_tamper_detection(tmp_path):
